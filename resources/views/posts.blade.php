    ...
    @if ($posts->count())
        <div class="card mb-3">
            @if ($posts[0]->image)
                ...
                @else
                <img src="/img/webprogramming1.jpg" class="card-img-top" alt="{{ $posts[0]->category->name }}" width="1200"
                height="400">
                @endif
            ...
            <div class="row">
                @foreach ($posts->skip(1) as $post)
                    <div class="col-md-4 mb-3">
                        <div class="card">
                            <div class="position-absolute px-3 py-2" style="background-color: rgba(0, 0, 0, 0.8)"><a
                                    href="/posts?category={{ $post->category->slug }}"
                                    class="text-white text-decoration-none">{{ $post->category->name }}</a></div>
                                    @if ($post->image)
                                        <img src="{{ asset('storage/' . $post->image) }}" class="img-fluid" alt="{{ $post->category->name }}">
                                    @else
                                    <img src="/img/personal1.jpg" class="card-img-top" alt="{{ $post->category->name }}">
                                    @endif
                            <div class="card-body">
